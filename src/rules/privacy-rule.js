...
const { ValidationRule, ValidationResult, SEVERITY, PRIVACY_PERMISSION_KEYS } = require('../utils/constants')
const path = require('path')
const fs = require('fs')

class PrivacyComplianceRule extends ValidationRule {
  constructor() {
    super('privacy-compliance', 'Validates privacy-related requirements including manifests and permissions')
  }
  ...
  async validatePrivacyManifest(validator, results) {
    ...
    if (validator.getBuildPath().endsWith('.app')) {
      privacyManifestPath = path.join(validator.getBuildPath(), 'PrivacyInfo.xcprivacy')
    } else {
    ...
    if (validator.getBuildPath().endsWith('.app') && !fs.existsSync(privacyManifestPath)) {
      results.push(new ValidationResult(
        this.name,
        SEVERITY.HIGH,
        'Privacy manifest file (PrivacyInfo.xcprivacy) not found',
  ...
  validatePrivacyManifestContent(manifest, results) {
    ...
    if (manifest.NSPrivacyTracking === true &&
        (!manifest.NSPrivacyTrackingDomains || manifest.NSPrivacyTrackingDomains.length === 0)) {
      results.push(new ValidationResult(
        this.name,
        SEVERITY.HIGH,
  ...
  validatePermissionStrings(plist, results) {
    ...
    for (const permissionKey of PRIVACY_PERMISSION_KEYS) {
      if (plist[permissionKey]) {
        ...
        if (genericPhrases.some(phrase => description.toLowerCase().includes(phrase.toLowerCase()))) {
          results.push(new ValidationResult(
            this.name,
            SEVERITY.MEDIUM,
        ...
        if (description.includes('TODO') || description.includes('CHANGE ME') || description.includes('...')) {
          results.push(new ValidationResult(
            this.name,
            SEVERITY.HIGH,
  ...
  validateLocationPermissions(plist, results) {
    ...
    const hasAlwaysAndWhenInUse = plist.NSLocationAlwaysAndWhenInUseUsageDescription
  ...
  validateDataCollection(plist, results) {
    ...
    const hasDataCollection = dataCollectionIndicators.some(key => plist[key])
