...
class FileParser {
  ...
  async parseIPA(ipaPath) {
    return new Promise((resolve, reject) => {
      yauzl.open(ipaPath, { lazyEntries: true }, (err, zipfile) => {
        if (err) { return reject(new Error(`Could not open IPA file: ${err.message}`)) }
        ...
        zipfile.on('entry', (entry) => {
          ...
          if (
            ...
            this.extractFileFromZip(zipfile, entry, (content) => {
              try {
                ...
                reject(
                  new Error(`Could not parse Info.plist: ${error.message}`)
                )
                return
              }
              zipfile.readEntry()
          ...
          if (fileName.includes('PrivacyInfo.xcprivacy')) {
            this.extractFileFromZip(zipfile, entry, (content) => {
              try {
                ...
                console.warn(
                  `Warning: Could not parse privacy manifest: ${error.message}`
                )
              }
              zipfile.readEntry()
            })
          ...
          if (
            ...
            this.extractFileFromZip(zipfile, entry, (content) => {
              try {
                ...
                console.warn(
                  `Warning: Could not parse entitlements: ${error.message}`
                )
              }
              zipfile.readEntry()
            })
        ...
        zipfile.on('end', () => {
          if (!artifacts.infoPlist) {
            reject(new Error('Info.plist not found in IPA file'))
            return
          }
          resolve(artifacts)
        })
  ...
  extractFileFromZip(zipfile, entry, callback) {
    zipfile.openReadStream(entry, (err, readStream) => {
      if (err) {
        console.warn(
          `Warning: Could not read ${entry.fileName}: ${err.message}`
        )
        zipfile.readEntry()
        return
      }
      ...
      readStream.on('error', (err) => {
        console.warn(
          `Warning: Error reading ${entry.fileName}: ${err.message}`
        )
        zipfile.readEntry()
      })
    })
  ...
  async parsePlist(filePath) {
    try {
      ...
      throw new Error(
        `Could not parse plist file ${filePath}: ${error.message}`
      )
    }
  }
  ...
  cleanup() {
    ...
    for (const tempFile of this.tempFiles) {
      try {
        ...
        console.warn(
          `Warning: Could not clean up temp file ${tempFile}: ${error.message}`
        )
      }
    }
    this.tempFiles = []
